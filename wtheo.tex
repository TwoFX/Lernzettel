...
\usepackage{dsfont}
\usepackage[left=0.8cm, right=0.8cm, top=0.6cm, bottom=0.6cm,landscape]{geometry}
...
\begin{multicols}{3}
	...
	\begin{satz}[Straffheit und Verteilungskonvergenz]
		\begin{enumerate}[label=(\alph*)]
			...
			\item $((X_n)~\text{straff} \land \exists Q~\forall (X_{n_k})~\text{vk}: X_{n_k}\vk Q)\implies X_n\vk Q$.
		\end{enumerate}
	\end{satz}
	...
	\begin{satz}[Notwendig für die Lindeberg-Bedingung]
		Feller-Bedingung:
		\[
			\lim_{n\to\infty}\frac{\max_{1\leq j\leq k_n}\sigma_{nj}^2}{\sigma_{n1}^2+\cdots+\sigma_{nk_n}^2} = 0.
		\]
	\end{satz}
	\begin{satz}[Ljapunov]
